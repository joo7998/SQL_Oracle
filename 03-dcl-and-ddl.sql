...
-- 사용자 생성
CREATE USER PARKMJ IDENTIFIED BY 1234;
-- 비밀번호 변경
ALTER USER PARKMJ IDENTIFIED BY test;
-- 사용자 삭제
DROP USER PARKMJ;
-- 경우에 따라 내부에 테이블 등 데이터베이스 객체가 생성된 사용자
DROP USER PARKMJ CASCADE; --  폭포수
...
-- 사용자 계정에게 접속 권한 부여
GRANT create session TO PARKMJ;
-- 일반적으로 데이터베이스 접속, 테이블 만들어 사용하려면
-- CONNECT, RESOURCE 롤을 부여
GRANT connect, resource TO PARKMJ;
-- Oracle 12이상에서는 사용자 테이블 스페이스에 공간 부여 필요
ALTER USER PARKMJ DEFAULT TABLESPACE USERS QUOTA unlimited ON USERS;
...
GRANT select ON hr.employees TO PARKMJ;
GRANT select ON hr.departments TO PARKMJ;
...
REVOKE select ON hr.departments FROM PARKMJ;
